...
export function VersionHistoryPanel({ artifactId, versions, onRestore }: VersionHistoryPanelProps) {
  ...
  return (
    <div className="flex h-full w-64 shrink-0 flex-col border-l border-border bg-surface">
      ...
      <div className="flex-1 overflow-auto p-2">
        {versions.length === 0 && <p className="p-2 text-xs text-text-muted">No saved versions yet.</p>}
        {versions.map((version, index) => (
          <div key={version.versionId} className="mb-2 rounded-md border border-border p-2 hover:border-accent">
